...
async function createUser(
  ...
  const params = {
    ...
    MessageAction: "SUPPRESS" as MessageActionType, // 招待メールを送信しない設定
    UserAttributes: [
  ...
  try {
    ...
    console.log(`ユーザー [${user.username}] を作成しました。`, response);
  } catch (error) {
    console.error(
      `ユーザー [${user.username}] の作成中にエラーが発生しました:`,
      error
    );
  }
}
...
async function main() {
  ...
  for (const user of users) {
    ...
    if (exists) {
      console.log(
        `ユーザー [${user.email}] は既に存在しています。スキップします。`
      );
      continue;
    }
    await createUser(client, user, userPoolId);
  }
}
