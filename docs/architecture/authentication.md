...
- 同時に個人グループを作成すること（フェーズ1で groups 実装時。現状は未実装）。
...
3. JIT プロビジョニングで一般ユーザーが作成されます（個人グループの自動作成は groups 実装時）。
...
| `401 Unauthorized` | `/` にローカルログインフォームを表示 |
| その他 | エラーメッセージ |
...
| `/api/v1/me` の結果 | フロントエンドの挙動 |
| --- | --- |
| `200 OK` | `/home` へリダイレクト（AppShell 内） |
| `401 Unauthorized` | `/` のログイン画面を表示 |
| その他 | エラーメッセージ |

認証済みの `/` アクセスは `/home` へリダイレクトします。
