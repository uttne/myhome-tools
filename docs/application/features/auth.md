...
| UC-AUTH-005 | Cloudflare Access 経由で初回ログインする | 許可メール | 実装済み |
...
| 画面 | パス | 概要 | 状態 |
| --- | --- | --- | --- |
| ホーム | `/` | 認証状態に応じてログインまたはログイン済み表示 | 実装済み |
| ログアウト | `/logout` | ログアウト API 実行後リダイレクト | 実装済み |
| パスワード設定 | TBD | ローカルパスワード設定・変更 | 未実装 |

### 認証判定

アプリ初期ロード時（`/`）に `GET /api/me` を呼び出します。

| 結果 | 挙動 |
| --- | --- |
| `200 OK` | ログイン済み画面（email / role / provider） |
...
/logout (LogoutPage)
└── POST /api/auth/logout → logout_url または /
```
...
| メソッド | パス | 概要 | 認証 |
| --- | --- | --- | --- |
| `GET` | `/api/me` | 現在のユーザー | Cloudflare / JWT |
| `POST` | `/api/auth/login` | ローカルログイン | 不要 |
| `POST` | `/api/auth/logout` | ログアウト | 不要 |
| `POST` | `/api/auth/password` | パスワード設定・変更 | ログイン済み |
...
| `PATCH` | `/api/admin/users/{id}` | 無効化、ロール変更 | admin |
| `POST` | `/api/admin/users/{id}/reset-password` | リセット要求 | admin |
...
- Cloudflare Access JWT 検証の実装タイミング → `../../open-questions.md`
- パスワード設定画面のパス → TBD
