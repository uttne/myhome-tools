...
| `backend.env.FILE_STORAGE_ROOT` | `/data/files` | ファイル保存のルートディレクトリです。本番では JuiceFS PVC のマウント先を指定します。 |
| `backend.fileStorage.enabled` | `false` | 既存 PVC を backend にマウントするかどうかです。 |
| `backend.fileStorage.mountPath` | `/data/files` | PVC のコンテナ内マウントパスです。`FILE_STORAGE_ROOT` と一致させます。 |
| `backend.fileStorage.existingClaim` | `""` | 家庭 k3s 管理リポジトリで作成した PersistentVolumeClaim 名です。 |
...
helm upgrade --install myhome-tools charts/myhome-tools \
  ...
  --set backend.fileStorage.existingClaim=myhome-juicefs-pvc
```

JuiceFS PVC の作成は家庭 k3s 管理リポジトリ側の責務です。
